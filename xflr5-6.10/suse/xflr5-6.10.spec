...
%build 
%if 0%{?fedora_version}  
qmake-qt4 -makefile %{name}_%{version}.pro 'target.path = %{buildroot}%{_bindir}'  
qmake-qt4 'target.path = %{buildroot}%{_bindir}'  
%else  
#qmake 'target.path = %{buildroot}%{_bindir}'  
qmake -makefile %{name}_%{version}.pro 'target.path = %{buildroot}%{_bindir}'  
qmake 'target.path = %{buildroot}%{_bindir}'  
#qmake
%endif  
...
%__install -m 644 doc/*.odt             %{buildroot}/usr/share/%{name} 
