			...
			while(!atEnd() && !hasError() && readNextStartElement() )
			{
				if (name().toString().compare("units", Qt::CaseInsensitive)==0)
				{
					while(!atEnd() && !hasError() && readNextStartElement() )
					{
						...
						else if (name().compare("speed_unit_to_ms",      Qt::CaseInsensitive)==0)
						{
							velocityunit = readElementText().toDouble();
						}
						else if (name().compare("inertia_unit_to_kgm2",  Qt::CaseInsensitive)==0)
						{
							inertiaunit = readElementText().toDouble();
						}
						else if (name().compare("area_unit_to_m2",  Qt::CaseInsensitive)==0)
						{
...
void XmlWPolarReader::readWPolar(WPolar *pWPolar, double lengthunit, double areaunit, double massunit, double velocityunit, double inertiaunit)
{
	...
	while(!atEnd() && !hasError() && readNextStartElement() && iw<MAXWINGS)
	{
		...
		else if (name().compare("Ignore_Body_Panels",         Qt::CaseInsensitive)==0)
		{
			pWPolar->bIgnoreBodyPanels() = stringToBool(readElementText());
		}
		else if (name().compare("Use_VLM1",         Qt::CaseInsensitive)==0)
		{
			pWPolar->bVLM1() = stringToBool(readElementText());
		}
		else if (name().compare("Viscous_Analysis",         Qt::CaseInsensitive)==0)
		{
			pWPolar->bViscous() = stringToBool(readElementText());
		}
		else if (name().compare("Thin_Surfaces",         Qt::CaseInsensitive)==0)
		{
			pWPolar->bThinSurfaces() = stringToBool(readElementText());
		}
		else if (name().compare("Wake_Rollup",         Qt::CaseInsensitive)==0)
		{
			pWPolar->bWakeRollUp() = stringToBool(readElementText());
		}
		else if (name().compare("Tilted_Analysis",         Qt::CaseInsensitive)==0)
		{
			pWPolar->bTilted() = stringToBool(readElementText());
		}
		else if (name().compare("Ground_Effect",         Qt::CaseInsensitive)==0)
		{
			pWPolar->bGround() = stringToBool(readElementText());
		}
		else if (name().compare("Ground_Height",         Qt::CaseInsensitive)==0)
		{
			pWPolar->groundHeight() = readElementText().toDouble()*lengthunit;
		}
		else if (name().compare("Viscosity",         Qt::CaseInsensitive)==0)
		{
			pWPolar->viscosity() = readElementText().toDouble();
		}
		else if (name().compare("Density",         Qt::CaseInsensitive)==0)
		{
			pWPolar->density() = readElementText().toDouble();
		}
		else if (name().compare("Fixed_Velocity",         Qt::CaseInsensitive)==0)
		{
			pWPolar->velocity() = readElementText().toDouble()*velocityunit;
		}
		else if (name().compare("Fixed_AOA",         Qt::CaseInsensitive)==0)
		{
			pWPolar->Alpha() = readElementText().toDouble();
		}
		else if (name().compare("Fixed_Bank_Angle",       Qt::CaseInsensitive)==0)
		{
			pWPolar->Phi() = readElementText().toDouble();
		}
		else if (name().compare("Fixed_SideSlip", Qt::CaseInsensitive)==0)
		{
			pWPolar->sideSlip() = readElementText().toDouble();
		}
		else if (name().compare("Reference_Dimensions", Qt::CaseInsensitive)==0)
		{
			pWPolar->referenceDim() = referenceDimension(readElementText());
		}
		else if (name().compare("Reference_Area", Qt::CaseInsensitive)==0)
		{
			pWPolar->referenceArea() = readElementText().toDouble()*areaunit;
		}
		else if (name().compare("Reference_Span_Length", Qt::CaseInsensitive)==0)
		{
			pWPolar->referenceSpanLength() = readElementText().toDouble()*lengthunit;
		}
		else if (name().compare("Reference_Chord_Length", Qt::CaseInsensitive)==0)
		{
			pWPolar->referenceChordLength() = readElementText().toDouble()*lengthunit;
		}
		else if (name().compare("ExtraDrag",         Qt::CaseInsensitive)==0)
		{
			while(!atEnd() && !hasError() && readNextStartElement() )
			{
				if (name().compare("ExtraDragCoef_1", Qt::CaseInsensitive)==0)
				{
					pWPolar->m_ExtraDragCoef[0] = readElementText().toDouble();
				}
				else if (name().compare("ExtraDragArea_1", Qt::CaseInsensitive)==0)
				{
					pWPolar->m_ExtraDragArea[0] = readElementText().toDouble();
				}
				else if (name().compare("ExtraDragCoef_2", Qt::CaseInsensitive)==0)
				{
					pWPolar->m_ExtraDragCoef[1] = readElementText().toDouble();
				}
				else if (name().compare("ExtraDragArea_2", Qt::CaseInsensitive)==0)
				{
					pWPolar->m_ExtraDragArea[1] = readElementText().toDouble();
				}
				else if (name().compare("ExtraDragCoef_3", Qt::CaseInsensitive)==0)
				{
					pWPolar->m_ExtraDragCoef[2] = readElementText().toDouble();
				}
				else if (name().compare("ExtraDragArea_3", Qt::CaseInsensitive)==0)
				{
					pWPolar->m_ExtraDragArea[2] = readElementText().toDouble();
				}
				else if (name().compare("ExtraDragCoef_4", Qt::CaseInsensitive)==0)
				{
					pWPolar->m_ExtraDragCoef[3] = readElementText().toDouble();
				}
				else if (name().compare("ExtraDragArea_4", Qt::CaseInsensitive)==0)
				{
		...
		else if (name().compare("Use_Plane_Inertia", Qt::CaseInsensitive)==0)
		{
			pWPolar->bAutoInertia() = stringToBool(readElementText());
		}
		else if (name().compare("Inertia_Value",         Qt::CaseInsensitive)==0)
		{
			while(!atEnd() && !hasError() && readNextStartElement() )
			{
				if (name().compare("Mass", Qt::CaseInsensitive)==0)
				{
					pWPolar->mass() = readElementText().toDouble()*massunit;
				}
				else if (name().compare("CoG", Qt::CaseInsensitive)==0)
				{
				...
				else if (name().compare("CoG_Ixx", Qt::CaseInsensitive)==0)
				{
					pWPolar->CoGIxx() = readElementText().toDouble()*inertiaunit;
				}
				else if (name().compare("CoG_Iyy", Qt::CaseInsensitive)==0)
				{
					pWPolar->CoGIyy() = readElementText().toDouble()*inertiaunit;
				}
				else if (name().compare("CoG_Izz", Qt::CaseInsensitive)==0)
				{
		...
		else if (name().compare("Inertia_gains",         Qt::CaseInsensitive)==0)
		{
			while(!atEnd() && !hasError() && readNextStartElement() )
			{
				if (name().compare("Mass_gain", Qt::CaseInsensitive)==0)
				{
					pWPolar->m_inertiaGain[0] = readElementText().toDouble()*massunit;
				}
				else if (name().compare("CoG_x_gain", Qt::CaseInsensitive)==0)
				{
					pWPolar->m_inertiaGain[1] = readElementText().toDouble()*lengthunit;
				}
				else if (name().compare("CoG_z_gain", Qt::CaseInsensitive)==0)
				{
					pWPolar->m_inertiaGain[2] = readElementText().toDouble()*lengthunit;
				}
				else if (name().compare("Ixx_gain", Qt::CaseInsensitive)==0)
				{
					pWPolar->m_inertiaGain[3] = readElementText().toDouble()*inertiaunit;
				}
				else if (name().compare("Iyy_gain", Qt::CaseInsensitive)==0)
				{
					pWPolar->m_inertiaGain[4] = readElementText().toDouble()*inertiaunit;
				}
				else if (name().compare("Izz_gain", Qt::CaseInsensitive)==0)
				{
					pWPolar->m_inertiaGain[5] = readElementText().toDouble()*inertiaunit;
				}
				else if (name().compare("Ixz_gain", Qt::CaseInsensitive)==0)
				{
		...
		else if (name().compare("Angle_gains",         Qt::CaseInsensitive)==0)
		{
			while(!atEnd() && !hasError() && readNextStartElement() )
			{
