...
void Wing::surfacePoint(double xRel, double ypos, enumPanelPosition pos, CVector &Point, CVector &PtNormal)
{
...
//	}
	...
	for (j=0; j<NSurfaces; j++)
	{
		if(m_Surface.at(j)->m_bIsTipLeft && !pWPolar->bThinSurfaces()) p+=m_Surface.at(j)->m_NXPanels;//tip patch panels
		...
		for (k=0; k<m_Surface.at(j)->m_NYPanels; k++)
		{
			...
			else
			{
				...
				{
					if(pWPolar->bVLM1() || m_pWingPanel[pp].m_bIsTrailing)
					{
...
//		if(s_bVLMSymetric) p+=m_Surface.at(j)->NXPanels * m_Surface.at(j)->NYPanels;
		if(m_Surface.at(j)->m_bIsTipRight && !pWPolar->bThinSurfaces()) p+=m_Surface.at(j)->m_NXPanels;//tip patch panels
	}
