...
void TwoDWidget::keyPressEvent(QKeyEvent *event)
{
	...
	if(pMainFrame->m_iApp == XFLR5::XFOILANALYSIS && m_pXDirect)
	{
		QXDirect* pXDirect = (QXDirect*)m_pXDirect;
		pXDirect->keyPressEvent(event);
	}
	else if(pMainFrame->m_iApp == XFLR5::MIAREX && m_pMiarex)
	{
		QMiarex* pMiarex = (QMiarex*)m_pMiarex;
		pMiarex->keyPressEvent(event);
	}
	else if(pMainFrame->m_iApp == XFLR5::DIRECTDESIGN && m_pAFoil)
	{
		QAFoil *pAFoil= (QAFoil*)m_pAFoil;
		pAFoil->keyPressEvent(event);
	}
	else if(pMainFrame->m_iApp == XFLR5::INVERSEDESIGN && m_pXInverse)
	{
...
//	MainFrame *pMainFrame = (MainFrame*)m_pMainFrame;
	...
	if(m_pMiarex)
	{
		...
		pMiarex->Set2DScale();
		if(pMiarex->m_iView==XFLR5::WOPPVIEW)   pMiarex->SetWingLegendPos();
		if(pMiarex->m_iView==XFLR5::WPOLARVIEW) pMiarex->SetWPlrLegendPos();
	}
	if(m_pAFoil)
	{
...
void TwoDWidget::contextMenuEvent (QContextMenuEvent * event)
{
	...
	switch(pMainFrame->m_iApp)
	{
		case XFLR5::MIAREX:
		{
			...
			if(pMiarex->m_iView==XFLR5::WOPPVIEW)            pMainFrame->WOppCtxMenu->exec(ScreenPt);
			else if (pMiarex->m_iView==XFLR5::WPOLARVIEW)    pMainFrame->WPlrCtxMenu->exec(ScreenPt);
			else if (pMiarex->m_iView==XFLR5::WCPVIEW)       pMainFrame->WCpCtxMenu->exec(ScreenPt);
			else if (pMiarex->m_iView==XFLR5::STABTIMEVIEW)  pMainFrame->WTimeCtxMenu->exec(ScreenPt);
			else if (pMiarex->m_iView==XFLR5::STABPOLARVIEW) pMainFrame->WPlrCtxMenu->exec(ScreenPt);
