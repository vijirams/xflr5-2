...
//	-- Wikipedia flight dynamics --
	...
	glNewList(VLMMOMENTS, GL_COMPILE);
	{
		...
		amp = 0.5*pWPolar->density() * pWPolar->referenceArea() * pWPolar->referenceSpanLength()
		...
		amp = 0.5*pWPolar->density() * pWPolar->referenceArea() * pWPolar->referenceChordLength()
						*pPOpp->m_QInf*pPOpp->m_QInf
						*pPOpp->m_GRm/3.0;
...
void GLCreatePanelForce(int nPanels, Panel *pPanel, WPolar *pWPolar, PlaneOpp *pPOpp)
{
	...
	rmin *= 0.5*pWPolar->density() *pPOpp->m_QInf*pPOpp->m_QInf  *QMiarex::s_LiftScale *coef;
	rmax *= 0.5*pWPolar->density() *pPOpp->m_QInf*pPOpp->m_QInf  *QMiarex::s_LiftScale *coef;
	range = rmax - rmin;
...
void GLCreateMesh(Wing *pWingList[MAXWINGS])
{
	QColor color;
	int width, j,l,k;
						...
						glBegin(GL_QUAD_STRIP);
						{
							for (l=0; l<pWing->m_Surface.at(j)->m_NXPanels; l++)
							{
								...
								glVertex3d(pWing->m_Surface.at(j)->TA.x,
										   pWing->m_Surface.at(j)->TA.y,
										   pWing->m_Surface.at(j)->TA.z);
								glVertex3d(pWing->m_Surface.at(j)->TB.x,
										   pWing->m_Surface.at(j)->TB.y,
										   pWing->m_Surface.at(j)->TB.z);
							}
							...
							for (l=pWing->m_Surface.at(j)->m_NXPanels-1; l>=0; l--)
							{
								...
								glVertex3d(pWing->m_Surface.at(j)->LA.x,
										   pWing->m_Surface.at(j)->LA.y,
										   pWing->m_Surface.at(j)->LA.z);
								glVertex3d(pWing->m_Surface.at(j)->LB.x,
										   pWing->m_Surface.at(j)->LB.y,
										   pWing->m_Surface.at(j)->LB.z);
							}
							glVertex3d(pWing->m_Surface.at(j)->TA.x,
									   pWing->m_Surface.at(j)->TA.y,
									   pWing->m_Surface.at(j)->TA.z);
...
void GLCreateSectionHighlight(int List, Wing *m_pWing, int highlightSection, bool bRightSide)
{
	...
	//skip zero-length surfaces
	for(int jSection=0; jSection<m_pWing->NWingSection(); jSection++)
	{
		if(jSection==highlightSection) break;
		if(qAbs(m_pWing->YPosition(jSection+1)-m_pWing->YPosition(jSection)) > Wing::minPanelSize())
			iSection++;
	}
